...
def filtrar_contornos(imagen, area_minima):
    """
    Esta función filtra los contornos de una imagen basada en un área mínima
    y devuelve una máscara con los contornos filtrados.
    """
    # Encontrar contornos
    contornos, _ = cv2.findContours(imagen, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)

    # Filtrar contornos
    dibujo = np.zeros_like(imagen)
    for contorno in contornos:
        area = cv2.contourArea(contorno)
        if area > area_minima:  # Ajusta el valor según sea necesario
            cv2.drawContours(dibujo, [contorno], -1, (255), thickness=cv2.FILLED)

    # Crear una máscara para el dígito
    mask = cv2.bitwise_and(imagen, imagen, mask=dibujo)
    
    return mask

def limpiar_digito(imagen):
    """
    Esta función toma una imagen de un dígito en blanco sobre fondo negro,
    la limpia eliminando contornos no deseados y devuelve la imagen procesada.
    """
    # Usar área mínima inicial
    area_minima = 1700  
    mask = filtrar_contornos(imagen, area_minima)

    # Volver a comprobar si la imagen es completamente negra
    if np.count_nonzero(mask) < 1:  # Si no hay píxeles blancos en la máscara
        print("La imagen es completamente negra. Cambiando área mínima a 100.")
        mask = filtrar_contornos(imagen, 100)
    

    # Aplicar dilatación si es necesario
    dilatacion = cv2.dilate(mask, np.ones((3, 3), np.uint8), iterations=1)
...
def procesar_imagen(img_path, output_dir):
    ...
    for i, caracter in enumerate(caracteres_ordenados):
        ...
        # Limpiamos la imagen
        caracter = limpiar_digito(caracter)
