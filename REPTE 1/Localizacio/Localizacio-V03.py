# -*- coding: utf-8 -*-
"""
Created on Thu Sep 19 19:53:59 2024
...
# Asegúrate de que la ruta de Tesseract esté correctamente configurada
...
def recortar_imagen_con_coordenadas(image, coords, margen=0):
    """
    Recorta la imagen usando las coordenadas proporcionadas y agrega un margen opcional.
    """
    x1, y1, x2, y2 = coords
    ...
    x1 = max(0, x1 - margen)
    y1 = max(0, y1 - margen)
    x2 = min(image.shape[1], x2 + margen)
...
def procesar_y_enderezar_letras(img_crop):
    ...
    # Extraer el canal V (valor) y aplicar Canny
    hsv = cv2.cvtColor(img_crop, cv2.COLOR_BGR2HSV)
    canal_v = hsv[:, :, 2]
    bordes_canny = cv2.Canny(canal_v, 30, 100)  # Ajusta estos valores
    ...
    # Enderezamiento (igual que antes)
...
def main():
    ...
    directorio_imagenes = os.path.dirname(os.path.abspath(__file__))  # Carpeta donde está el archivo
    ...
    for nombre_archivo in os.listdir(directorio_imagenes):
        if nombre_archivo.lower().endswith(extensiones):
            img_path = os.path.join(directorio_imagenes, nombre_archivo)
            ...
            if best_detection is not None:
                ...
                # Enderezar la imagen de la matrícula
                img_enderezada = procesar_y_enderezar_letras(img_crop)

                # Volver a aplicar YOLO sobre la imagen enderezada
                detections_enderezada = detect_objects(model, img_enderezada)
                best_detection_enderezada = get_best_detection(detections_enderezada)

                if best_detection_enderezada is not None:
                    # Obtener las coordenadas
                    coords_enderezada = list(map(int, best_detection_enderezada[:4]))

                    # Recortar la imagen final
                    img_final_crop = recortar_imagen_con_coordenadas(img_enderezada, coords_enderezada, margen=50)
                else:
                    # Si no hay detección, usar la imagen enderezada directamente
                    img_final_crop = img_enderezada
                
                # Procesar la imagen final
                img_final_enderezada = procesar_y_enderezar_letras(img_final_crop)

                # Guardar la imagen en el directorio especificado
                img_output_path = os.path.join(directorio_imagenes, 'imatges_matriculas', nombre_archivo)
                if not os.path.exists(os.path.dirname(img_output_path)):
                    os.makedirs(os.path.dirname(img_output_path))

                cv2.imwrite(img_output_path, img_final_enderezada)
                print(f"Imagen guardada en: {img_output_path}")
                
            else:
                print(f"No se detectó ninguna matrícula en la imagen original: {nombre_archivo}")
