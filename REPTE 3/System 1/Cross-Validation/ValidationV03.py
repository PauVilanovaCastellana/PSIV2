...
def evaluate_patch_classification(k_patient_ids, k_labels, data_folder, model):
    roc_save_path = "/export/fhome/maed01/myvirtualenv/AutoEncoder/Images/PatchClassification_k_154_Validation_micro.png"
    boxplot_save_path = "/export/fhome/maed01/myvirtualenv/AutoEncoder/Images/F_Red_Distribution_Boxplot_micro.png"
...
def evaluate_patient_diagnosis(k_patient_ids, k_labels, data_folder, model, patch_threshold):
    save_path= "/export/fhome/maed01/myvirtualenv/AutoEncoder/Images/PatientDiagnosis_k_154_Validation_micro.png"
    """
...
def cross_validate_system(patient_ids, labels, data_folder, model, n_splits=15, save_excel_path="/export/fhome/maed01/myvirtualenv/AutoEncoder/Images/Cross_Validation_Metrics_with_Thresholds_Validation06.csv"):
    ...
    avg_metrics = metrics_df[["recall", "precision", "f1"]].mean()
    std_metrics = metrics_df[["recall", "precision", "f1"]].std()
    ...
    print(f"patch_auc: {patch_auc_mean:.4f} ± {patch_auc_std:.4f}")
    print(f"patient_auc: {patient_auc_mean:.4f} ± {patient_auc_std:.4f}")

    # Generar gráfica de métricas promedio y desviación estándar
    metrics_names = list(avg_metrics.index) + ["patch_auc", "patient_auc"]
    avg_values = list(avg_metrics.values) + [patch_auc_mean, patient_auc_mean]
    std_values = list(std_metrics.values) + [patch_auc_std, patient_auc_std]
    ...
    plt.tight_layout()
    plt.savefig("/export/fhome/maed01/myvirtualenv/AutoEncoder/Images/Average_Metrics_Validation05.png")
    plt.close()
    print("Metrics plot saved to /export/fhome/maed01/myvirtualenv/AutoEncoder/Images/Average_Metrics_Validation05.png")

    # Guardar métricas en un archivo Excel
    metrics_df.to_excel(save_excel_path, index=False)
