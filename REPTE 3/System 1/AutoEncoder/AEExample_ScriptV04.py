...
# Redimensionar imágenes a tamaño fijo y convertirlas en tensor
patches = []
for image_path in cropped_images:
    image = cv2.imread(image_path)
    if image is None:
        continue
    # Convertir la imagen a RGB y redimensionarla a 256x256
    image_rgb = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)
    resized_image = cv2.resize(image_rgb, (256, 256))  # Redimensionar a (256, 256)
    
    # Normalizar y agregar a la lista de parches
    normalized_image = resized_image / 255.0
    patches.append(normalized_image)

# Definir el dataset como antes
class PatchesDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        image_tensor = image_tensor.permute(2, 0, 1)  # Cambiar la forma a [C, H, W]
        return image_tensor
...
# Guardar Model
model_path = 'autoencoder_model.pth'  # Nombre y ruta del archivo donde se guardará el modelo
