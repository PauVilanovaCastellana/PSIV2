# -*- coding: utf-8 -*-
"""
Created on Fri Jan 10 08:18:46 2025
...
def prepare_sequence_data(patient_ids, patient_data, sequence_length):
    ...
    for patient_id in patient_ids:
        ...
        max_sequences = 2500  # YYYYYYYYYY 5000
...
if __name__ == "__main__":
    ...
    for fold, (train_idx, test_idx) in enumerate(skf.split(all_eeg_data, all_labels)):
        ...
        plt.ylabel("Loss")
        plt.legend()
        plt.savefig(f"{RESULTS_DIR}/loss_curves/Solapament 10 k 2 Folds/loss_curve_fold_{fold + 1}.png")
        plt.close()
        ...
        plt.ylabel("Real")
        plt.savefig(f"{RESULTS_DIR}/confusion_matrix/Solapament 10 k 2 Folds/confusion_matrix_fold_{fold + 1}.png")
        plt.close()
    ...
    plt.tight_layout()
    plt.savefig(f"{RESULTS_DIR}/confusion_matrix/Solapament 10 k 2 Folds/metrics_summary.png")
    plt.close()
    ...
    plt.xlabel("Predicted")
    plt.ylabel("Real")
    plt.savefig(f"{RESULTS_DIR}/confusion_matrix/Solapament 10 k 2 Folds/confusion_matrix_total.png")
    plt.close()
